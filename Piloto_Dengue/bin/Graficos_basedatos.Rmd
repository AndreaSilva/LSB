---
title: "Graphics: Dengue virus database"
output: html_notebook
---

These graphs describe the Dengue virus database "bd_Dengue", which has the information collected from GenBank for all Dengue cases registered in this platform until September 2016. For more information about what tools were used to create the base of data and the contents of its variables can read the file [README] (https://gitlab.com/AndreaSilva/LSB/tree/master/Piloto_Dengue/data/Base_Datos/Base_Datos_Dengue)  
...
plot_serotype <- ggplot(data = serotype[3:7,], aes(x= Name_serotype, y = count_data))+
  ...
  labs(x= "Serotipos", y= "Número de registros")+
  ggtitle("Número de registros por Serotipos")+
  theme_minimal()+
  theme(plot.title = element_text(hjust=0.5))+
  scale_x_discrete(labels=c("DENV","DENV1", "DENV2", "DENV3", "DENV4"))+
...
pgenes <- ggplot(data = Genes.length, aes(x = gene_name, y = gene_count, color=gene_name)) +
  geom_bar(stat = "identity", fill="white")+
  labs(x= "Gen-es", y= "Número de Registros")+
  scale_color_grey(start= 0.7, end=0.1)+
  ggtitle("Número de registros por Gen-es")+
  theme_classic()+
  theme(plot.title = element_text(hjust=0.5))+
...
**Figura 3.** Gen-es reportados en la base de datos y número de regitros que presenta cada partición. La mayoria de los registros pertenecen a secuencias del Gen E y del Genoma, seguidos de las secuencias denominadas "gen de poliproteína" que presentan cuatro o mas genes. De los genes no estrulturales el gen no estructural 1 "NS1" presenta mayor número de regitros.
.
...
```{r, fig.width = 12, fig.height = 22, echo = FALSE}
...
#class(country.length$Country_names)

pp1 <- ggplot(data = country.length, aes(x = Country_names, y = number_cases, color=Country_names)) +
  geom_bar(stat = "identity", fill="white")+
  labs(x= "Países", y= "Número de Registros")+
  scale_color_grey(start= 0.7, end=0.1)+
  ggtitle("Número de registros por países")+
  theme_classic()+
  theme(plot.title = element_text(hjust=0.5))+
  theme(legend.position="none")+
  coord_flip()+
...
pf <- ggplot()+
  ...
  scale_size(name="Número de registros")+
  ggtitle("Número de registros por países")+
  theme_tufte()+
  theme(plot.title = element_text(hjust=0.5))+
...
axis.title.y = element_text(size=13, face="bold"))+
  theme(legend.text = element_text(size= 13, colour = "black", face = "bold"))+
...
for(i in 1:length(Country_names)){
  ...
  data.mundi1[i, "Serotipos"] <- mini.data$Serotype[1]
...
for(i in 1:length(Country_names)){
  ...
  data.mundi2[i, "Serotipos"] <- mini.data$Serotype[1]
...
for(i in 1:length(Country_names)){
  ...
  data.mundi3[i, "Serotipos"] <- mini.data$Serotype[1]
...
for(i in 1:length(Country_names)){
  ...
  data.mundi4[i, "Serotipos"] <- mini.data$Serotype[1]
...
for(i in 1:length(Country_names)){
  ...
  mundi1[i, "Serotipos"] <- mini.data$Serotype[1]
...
pg <- ggplot()+
  geom_map(data=map.world, map=map.world, aes(map_id=region, x=long, y=lat), fill="white", colour="black", size=0.5)+
  geom_point(data= data.mundi, aes(x=lon, y=lat, color=Serotipos),size=1.5, alpha=0.5)+
  scale_color_manual(values=c("purple","deeppink","green3","turquoise3","mediumblue"))+
  scale_color_manual(labels = c("DENV", "DENV1", "DENV2", "DENV3", "DENV4"), values = c("purple","deeppink","green3","turquoise3","mediumblue"))+
  ggtitle("Distribución de los serotipos por países")+
  theme_tufte()+
  theme(plot.title = element_text(hjust=0.5))+
  theme(plot.title = element_text(size=18, face = "bold"),
  axis.title.x = element_text(size=14, face="bold"),
...
#"green3","deeppink","turquoise3","mediumblue","purple"
#"purple","green3","deeppink","turquoise3","mediumblue"
```
...
for(i in 1:length(names_location)){
  ...
  SS[i, "Serotipos"] <- mini.data$Serotype[1]
...
for(i in 1:length(names_location)){
  ...
  S1[i, "Serotipos"] <- mini.data$Serotype[1]
...
for(i in 1:length(names_location)){
  ...
  S2[i, "Serotipos"] <- mini.data$Serotype[1]
...
for(i in 1:length(names_location)){
  ...
  S3[i, "Serotipos"] <- mini.data$Serotype[1]
...
for(i in 1:length(names_location)){
  ...
  S4[i, "Serotipos"] <- mini.data$Serotype[1]
...
pl <-ggplot()+
  geom_map(data=map.world, map=map.world, aes(map_id=region, x=long, y=lat), fill="white", colour="black", size=0.5)+
  geom_point(data= mundi.location, aes(x=lon, y=lat, color=Serotipos),size=1.5, alpha=0.5)+
  scale_color_manual(values=c("purple","deeppink","green3","turquoise3","mediumblue"))+
  scale_color_manual(labels = c("DENV", "DENV1", "DENV2", "DENV3", "DENV4"), values = c("purple","deeppink","green3","turquoise3","mediumblue"))+
  ggtitle("Distribución de los serotipos por localidades")+
  theme_tufte()+
  theme(plot.title = element_text(hjust=0.5))+
  theme(plot.title = element_text(size=18, face = "bold"),
  axis.title.x = element_text(size=14, face="bold"),
...
ggplot(data = count_years, aes(x = Year, y = number_year, color=Year)) +
  ...
  ggtitle("Número de registros por año")+
...
```
**Figura 12.** Frecuencia de los años registrados en la base de datos, siendo el registro mas antiguo del año 1944, y el mas reciente del año 2016. En la grafica se puede observar como en los años mas recientes se ha aumnetado el numero de casos de Dengue que son secuenciados y reportados, cabe recordar que la base de datos fue descargada en el 2016 por lo cual es muy probable que los datos que se tienen de los años 2015 y 2016 no representen la cantidad con la que se cuente en este momento para esos años. Tambien es importante resaltar que mas de 4.000 casos no reportan en año.
...
arsd <- ggplot(data = years_denv, aes(x = Year, y = number_year, color=Year)) +
  geom_bar(stat = "identity", fill="white")+
  labs(x= "Años", y= "Número de registros")+
  scale_color_grey(start= 0.1, end=0.7)+
  theme_classic()+
  theme(legend.position="none")+
  ggtitle("Años de registros con serotipo desconocido")+
  theme(plot.title = element_text(hjust=0.5))+
  theme(legend.position="none")+
  coord_flip()+
...
arsd
```
**Figura 13.** Años de registros con serotipo desconocido. Alrededor de la mitad de los registros con serotipos desconocido no reportan su año. En la **Figura 8.** se muestra que estos casos no pertenecen a un solo pais, y en esta figura vemos que tampoco pertenecen a un año especifico, y que tampoco pertenecen a casos viejos o nuevos.
...
ggplot(data = years_location, aes(x = Year, y = number_year, color=Year)) +
  ...
  theme_classic()+
  theme(legend.position="none")+
  ggtitle("Años de registros que reportan localidad")+
  coord_flip()

```
**Figura 14.** Años para casos con localidad. 6084 casos que reportan localidad, de los cuales 404 no reportan año.
...
```
**Figura 15.** Años para casos con pais y sin localidad. 12305 casos que reportan pais pero no localidad, de los cuales 3881 no reportan año.

Para las graficas **Figura 14.** y **Figura 15.** a medida que avanzan los años fue aumentando el número de casos que se reportan con pais o con pais-localidad.
